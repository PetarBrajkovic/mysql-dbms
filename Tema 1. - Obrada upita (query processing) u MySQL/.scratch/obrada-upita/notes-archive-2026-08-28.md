# NOTES.md as it stood on 2026-08-28, before it was split up

Archived, not current. This is the append-only log `NOTES.md` had become: 27 KB, read in full on
every lesson, mixing durable preferences with dated per-chapter findings and corrections stacked
under the claims they corrected.

It was split three ways: durable preferences stayed in `NOTES.md`, per-chapter findings were already
duplicated in `learning-records/` (and their standing constraints are now indexed in
`learning-records/README.md`), and the figure-script traps moved to `figures/README.md`.

Kept only so a sentence written before the split can be traced. Nothing here is authoritative;
where it disagrees with a current file, the current file wins. Older learning records still refer to
"`NOTES.md`" in their narrative — they mean this file.
...
- **Chapter 4c taught 2026-08-28 (lesson `0006`), and chapter 4 is now taught end to end.** Five
  things that change what other chapters may claim:
  - **The lesson-4b bad plan was never costed against anything.** `considered_execution_plans` holds
    exactly **one** access path (the table scan, ≈574.800) and `idx_created_at` is not in it at all;
    a later step, `reconsidering_access_paths_for_index_ordering`, swaps the plan with
    `"steps": []`, i.e. **without computing a single cost**. So `EXPLAIN`'s `0,838` is a consequence
    computed after the swap, not the reason for it. The paper must not write "the optimizer costed
    it wrong" - it never costed it. Removing `LIMIT` gives `plan_changed: false`; `LIMIT 10000` still
    swaps, so the trigger is the **existence** of `LIMIT`, not its size. This also retroactively
    explains LR-0005 (f): the histogram improved an estimate that was never used to decide.
  - **Trace an `EXPLAIN`, not the query.** The third phase is then `join_explain` instead of
    `join_execution`, `join_optimization` is identical with every cost, and nothing is executed. Use
    this whenever a chapter needs a trace of something slow; it made all of 4c instant.
  - **The trace's three phases are NOT chapter 3's five stages.** Parsing finishes before the trace
    starts, and optimization and planning are one block in it. Lesson 0006 said "the same ones" in a
    first draft and had to be corrected. Do not let chapters 5-8 repeat that.
  - **Default `optimizer_trace_max_mem_size` is 1048576 on this server, not 16 KB.** Research memo
    `05-explain-semantics.md` §4.4 says 16 KB; that is stale. Third error found in that memo.
  - **`EXPLAIN FOR CONNECTION` has five outcomes, and two get conflated.** Idle connection
    (`COMMAND: Sleep`) returns an **empty result, no error** (manual, and verified); a connection
    **busy with a non-explainable statement** (`DO SLEEP(...)`) returns **`ERROR 3012`**. Plus 1235
    with `ANALYZE`, 1094 for an unknown id, and 1295 through `PREPARE`. The 1295 case is **measured,
    not the manual's claim** - the manual documents the opposite direction (the *target* cannot be a
    prepared statement). `Com_explain_other` only moves in `SHOW GLOBAL STATUS`, since every
    `mysql -e` is a fresh session.
- **Terminology near-miss worth not repeating (2026-08-28):** lesson 0006 was written throughout with
  **`put pristupa`** for *access path*, by false analogy with §2c's `tip pristupa`. `GLOSSARY.md` §1
  already locks **`pristupni put`** (deck-derived). Caught before the chapter was written and
  reverted across the lesson, the reference card, `tools/make-lesson06-*.ps1` and two `examples/`
  files. Recorded in §2e because the analogy is tempting and chapter 5 will meet it again.
- **A quote attributed to the manual was written from memory and was wrong (2026-08-28).** Lesson
  0006's `.slide` block originally carried an invented paraphrase of the optimizer-tracing page. The
  page's actual opening sentence is dry and does not make the claim the lesson needed. Fixed by
  quoting the real sentence and making the teaching claim in the lesson's own voice. **Fetch the page
  before quoting it**, every time - `WORKFLOW.md` rule 6 covers `references.bib`, but the same rule
  has to apply to `.slide` blocks in lessons.
- **Three PowerShell traps in `tools/make-lesson06-optimizer-trace.ps1`**, beside the two from 4b:
  `$args` is an **automatic variable** (assigning to it broke `Start-Process` silently);
  `Start-Process -ArgumentList` **splits on spaces**, so a credentials path with spaces needs its own
  embedded quotes; and **`2>&1` on a native command throws** under `$ErrorActionPreference='Stop'`,
  so the helper that deliberately captures MySQL `ERROR` lines must relax the preference around the
  call. Also, MySQL-side: a plain sentinel `SELECT` used to split script output **is itself traced**
  and, with `optimizer_trace_limit = 1`, overwrites the trace you came for.
- **Chapter 4's page budget raised 6 → 6.6 (user's decision, 2026-08-28).** This closes the
  question the 4b entry above said to put to the user at the start of the 13c session. 4a came in
  at ≈3,2 pages and 4b at ≈2,4, so 13c gets its full ~1 page rather than being squeezed into the
  ~0,4 that were left. Reason: lesson 0006 produced the chapter's strongest finding (the bad plan
  was never costed against anything; `0,838` is a consequence of the `ORDER BY`+`LIMIT` swap, not
  its cause), and the only way to fit ~0,5 pages would have been to drop either that or
  `EXPLAIN FOR CONNECTION`. `GLOSSARY.md` §4 now totals ≈23,6 pages. The standing rule stands:
  **never trim written prose to make room.**
